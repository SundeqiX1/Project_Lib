...
#define         S_FUNCTION_NAME         pci1756IO
...
/* Read in Digital Input from Hardware */
   tempPortData =rl32eInpB((unsigned short)(BASE+0))&0xff;
   tempPortData1 =rl32eInpB((unsigned short)(BASE+1))&0xff;
   tempPortData2 =rl32eInpB((unsigned short)(BASE+2))&0xff;
   tempPortData3 =rl32eInpB((unsigned short)(BASE+3))&0xff;
      for (k=0;k<8;k++) {
//         y=ssGetOutputPortSignal(S,i);
        y[k] = (tempPortData >> k)&0x01;
        y[k+8]=(tempPortData1 >> k)&0x01;
           } 
     for (i=16;i<24;i++) {
        // z=ssGetOutputPortSignal(S,i);
        z[i-16] = (tempPortData2 >> (i-16))&0x01;
        z[i-8]=(tempPortData3 >> (i-16))&0x01;
          } 
