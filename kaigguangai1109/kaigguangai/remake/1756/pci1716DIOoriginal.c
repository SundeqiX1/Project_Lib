...
#define         S_FUNCTION_NAME         pci1716DIO
...
for(i=0;i<8;i++){        /* CHANNEL n'S GAIN SETTING */
...
//    rl32eOutpB( BASE + 5,i+1);     //Stop channel
  ...
  // rl32eOutpB((unsigned short)(BASE+2),(unsigned short)(0x24));          /* SETTING differential AND BIPOLAR AND GAIN = 1 */
   rl32eOutpB((unsigned short)(BASE+2),(unsigned short)(0x04));          /* SETTING single end AND BIPOLAR AND GAIN = 1 */
   rl32eOutpB((unsigned short)(BASE+6),(unsigned short)(0x01));          /* SETTING Software trigger */
 
   //output signal initialize
   rl32eOutpB((unsigned short)(BASE+14),(unsigned short)(0x01));          /* SETTING register for D/A refrence control */
   rl32eOutpB((unsigned short)(BASE+15),(unsigned short)(0x01));          /* SETTING register for D/A refrence control */
...
//output signals
 ...
 for (j=0;j<2;j++) {
    uPtrs = ssGetInputPortRealSignalPtrs(S,j);
    out=32768*(1-*uPtrs[0]/(-10));
    out=max(out,0);
    out=min(out,65535);
    out=floor(out+0.5);
    rl32eOutpW((unsigned short)(BASE+12),(unsigned short)out);
//      DA_Output(S, j, *uPtrs[0], -10);
    }
//output end
 
/* Read in Digital Input from Hardware */
   tempPortData =rl32eInpB((unsigned short)(BASE+17))&0xff;
   tempPortData1 =rl32eInpB((unsigned short)(BASE+16))&0xff;
      for (k=0;k<8;k++) {
...
/* Write Digital to Hardware */
  ...
  for (n=10;n<18;n++) {
    ...
    rl32eOutpB(BASE+17,DOPortData1);
