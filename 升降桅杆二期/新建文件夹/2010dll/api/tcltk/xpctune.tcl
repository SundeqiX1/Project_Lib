...
# $Revision: 1.1.6.2 $  $Date: 2004/07/30 02:24:48 $
...
proc xpctune { {blkparam  {Gain1 Gain} } } {
	...
	xpctcpconnect {144.212.20.171} {22222}
    ...
    # Arbitrarily use first parameter when parameter is an array
	set valueparam [lindex [$target_obj GetParam $index] 0] 
	...
	scale  .t.scale		-showvalue 1 	-orient horizontal  -length 180\
		-variable valueparam \
		-to [expr $valueparam * 2 + 1] \
        -from [expr $valueparam / 2 - 1]  \
        -resolution 0 \
...
proc tunecommand {index newval} {
	global target_obj
    set allparam [$target_obj GetParam $index]
    set allparam [lreplace $allparam 0 0 $newval]
	$target_obj SetParam $index allparam
}
