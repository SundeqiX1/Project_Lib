 ...
 * $Revision: 1.5.6.19 $ $Date: 2010/02/08 22:42:34 $
 */
...
int xPCInitAPI(void) {
    ...
    if ((dllHandle = GetModuleHandle(XPCAPIDLL)) == NULL)
        /* If the dll is not already loaded (dllHandle == NULL), *
         * call LoadLibrary to load it.                          */
        if ((dllHandle = LoadLibrary(XPCAPIDLL)) == NULL) {
            return -1;
        }
